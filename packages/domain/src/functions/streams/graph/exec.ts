...
// execs the spec.
// we call the root (AKA) the end node first which makes a recursive call downstream.

export type NodeEnv<K, D, W> = {
  readonly stream: Stream.Stream<IZSet<K, D, W>>
}

// suppose for now that I have or will have some method fro traversing the tree
// from the leafs to the root.
// now I need some sort of global track of streams, which are the only
// state that I have a view into.
